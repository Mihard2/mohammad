...
<div class="clear"></div>
<br />
<a href="https://wp.bulk-editor.com/document/filters/" target="_blank" class="button button-primary wpbe-info-btn"><span class="icon-book"></span>&nbsp;<?php esc_html_e('Documentation', 'bulk-editor') ?></a>
<br />
...
function wpbe_filter_draw_text() {
    ...
    $behavior_options = array(
        ...
        'exact' => esc_html('EXACT', 'bulk-editor'),
        'not' => esc_html('NOT', 'bulk-editor'),
        'begin' => esc_html('BEGIN', 'bulk-editor'),
        'end' => esc_html('END', 'bulk-editor'),
    );


    $filter_keys = array(
        'post__in' => array(
            'placeholder' => esc_html('ID(s). Use comma or/and minus for range', 'bulk-editor'),
            'behavior_options' => array('exact' => esc_html('EXACT', 'bulk-editor'))
        ),
        'post_title' => array(
            'placeholder' => sprintf(esc_html('[%s] title ...', 'bulk-editor'), $WPBE->settings->current_post_type),
            'behavior_options' => $behavior_options
        ),
        'post_content' => array(
            'placeholder' => sprintf(esc_html('[%s] content ...', 'bulk-editor'), $WPBE->settings->current_post_type),
            'behavior_options' => $behavior_options
        ),
        'post_excerpt' => array(
            'placeholder' => sprintf(esc_html('[%s] excerpt ...', 'bulk-editor'), $WPBE->settings->current_post_type),
            'behavior_options' => $behavior_options
        ),
        'post_name' => array(
            'placeholder' => sprintf(esc_html('[%s] slug ...', 'bulk-editor'), $WPBE->settings->current_post_type),
            'behavior_options' => $behavior_options
        ),
        'post_password' => array(
            'placeholder' => sprintf(esc_html('[%s] password ...', 'bulk-editor'), $WPBE->settings->current_post_type),
            'behavior_options' => $behavior_options
        ),
    );
    ...
    <div class='filter-unit-wrap wpbe_filter_draw_other'>
        ...
        <div class="col-lg-6">
            <div>
                ...
                echo WPBE_HELPER::draw_select(array(
                    'options' => array(-1 => sprintf(esc_html('[%s] status', 'bulk-editor'), $WPBE->settings->current_post_type)) + $fields['post_status']['select_options'],
                    'field' => 'post_status',
                    'post_id' => 0,
        ...
        <div class="col-lg-6">
            <div>
                <?php
                echo WPBE_HELPER::draw_select(array(
                    'options' => array(-1 => esc_html('Comment status', 'bulk-editor')) + $fields['comment_status']['select_options'],
        ...
        <div class="col-lg-6">
            <div>
                <?php
                echo WPBE_HELPER::draw_select(array(
                    'options' => array(-1 => esc_html('Ping status', 'bulk-editor')) + $fields['ping_status']['select_options'],
                    'field' => 'ping_status',
                    'post_id' => 0,
                    'class' => 'wpbe_filter_select',
        ...
        <?php if (in_array($WPBE->settings->current_post_type, $fields['post_mime_type']['allow_post_types'])): ?>
            ...
            <div class="col-lg-6">
                <div>
                    <?php
                    echo WPBE_HELPER::draw_select(array(
                        'options' => array(-1 => esc_html('Post mime type', 'bulk-editor')) + $fields['post_mime_type']['select_options'],
        ...
        <?php if (in_array($WPBE->settings->current_post_type, $fields['sticky_posts']['allow_post_types'])): ?>
            <div class="col-lg-6">
                <div>
                    <?php
                    echo WPBE_HELPER::draw_select(array(
                        'options' => array(-1 => esc_html('Sticky posts', 'bulk-editor')) + $fields['sticky_posts']['select_options'],
                        'field' => 'sticky_posts',
                        'post_id' => 0,
                        'class' => 'wpbe_filter_select',
        ...
        <div class="col-lg-6">
            <div>
                ...
                echo WPBE_HELPER::draw_select(array(
                    'options' => array('-1' => esc_html('By author', 'bulk-editor')) + $users, //+ $fields['author']['select_options'],
                    'field' => 'post_author',
                    'post_id' => 0,
                    'class' => 'wpbe_filter_select',
        ...
        <?php foreach ($calendar_fields as $ff) : ?>
            ...
            <?php if ($fields[$ff]['field_type'] === 'meta'): ?>
                ...
                <div class="col-lg-6">
                    <div>
                        <?php
                        echo WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_from", sprintf(esc_html('%s from', 'bulk-editor'), $fields[$ff]['title']), $ff . '_from', '', "wpbe_filter[{$ff}][from]", true);
                        ?>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div>
                        <?php
                        echo WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_to", sprintf(esc_html('%s to', 'bulk-editor'), $fields[$ff]['title']), $ff . '_to', '', "wpbe_filter[{$ff}][to]", true);
                        ?>
                    </div>
                </div>
            ...
            <?php else: ?>
                ...
                <div class="col-lg-6">
                    <div>
                        <?php
                        echo WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_from", sprintf(esc_html('%s from', 'bulk-editor'), $fields[$ff]['title']), $ff . '_from', '', "wpbe_filter[{$ff}_from]", true);
                        ?>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div>
                        <?php
                        echo WPBE_HELPER::draw_calendar("wpbe_filter_{$ff}_to", sprintf(esc_html('%s to', 'bulk-editor'), $fields[$ff]['title']), $ff . '_to', '', "wpbe_filter[{$ff}_to]", true);
                        ?>
                    </div>
                </div>
                    ...
                    <div class="wpbe_filter_other_item" style="padding-<?php echo $padding ?>: 1px !important;">
                        ...
                        echo WPBE_HELPER::draw_select(array(
                            'options' => array(
                                ...
                                'zero' => esc_html('No', 'bulk-editor'), //false
                            ),
                            'field' => $key,
                            'post_id' => 0,
                            'class' => 'wpbe_filter_select',
