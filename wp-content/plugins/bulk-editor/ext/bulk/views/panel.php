...
<div class="notice notice-warning">
    <p>
        <?php printf(esc_html('Bulk editing will be applied to: %s', 'bulk-editor'), '<span class="wpbe_action_will_be_applied_to">' . sprintf(esc_html('all the [%s] on the site', 'bulk-editor'), $WPBE->settings->current_post_type) . '</span>') ?>
    </p>
</div>
...
<div class="wpbe_delete_wraper wpbe-float-right">
    ...
    <?php if ($WPBE->show_notes): ?>
        <label><?php esc_html_e('Bulk deleting', 'bulk-editor') ?>  (<small class="wpbe-free-version"><?php esc_html_e('premium version', 'bulk-editor') ?></small>)</label><br />
    <?php else: ?>
        <label for="wpbe_bulk_delete_posts_btn_fuse"><?php esc_html_e('Bulk deleting', 'bulk-editor') ?></label><br />
    <?php endif; ?>

    <a href="#" disabled='disabled' class="button button-primary button-large" id="wpbe_bulk_delete_posts_btn" ><?php printf(esc_html('Delete [%s]!', 'bulk-editor'), $WPBE->settings->current_post_type) ?></a>
...
<ul>
    ...
    <li>* <?php
        printf(esc_html('Time by time (one time per week for example) - make the backup of your site database. For example by %s', 'bulk-editor'), WPBE_HELPER::draw_link(
                        array(
    ...
    <li>
        ...
        <a href="https://wp.bulk-editor.com/document/wordpress-posts-bulk-edit/" target="_blank" class="button button-primary wpbe-info-btn"><span class="icon-book"></span>&nbsp;<?php esc_html_e('Documentation', 'bulk-editor') ?></a>
...
function wpbe_bulk_draw_text($bulk_fields) {
    ...
    <?php foreach ($bulk_fields as $field_key => $field) : ?>
        ...
        <?php
        if ($WPBE->settings->current_user_role != 'administrator') {
...
function wpbe_bulk_draw_nums($filter_keys) {
    ...
    <div class="col-lg-1">
        <?php
        WPBE_HELPER::draw_tooltip(sprintf(esc_html('Select how to round float values fractions in the numeric fields. Works for numeric meta fields. Read more %s.', 'bulk-editor'), WPBE_HELPER::draw_link(array(
                            'title' => esc_html('here', 'bulk-editor'),
                            'href' => 'https://wp.bulk-editor.com/document/rounding/',
                            'target' => '_blank'
        ))))
        ?>
    </div>
...
function wpbe_bulk_draw_taxonomies() {
    ...
    if (!empty($taxonomy_objects)) {
        ...
        foreach ($taxonomy_objects as $t) {
            ...
            if ($WPBE->settings->current_user_role != 'administrator') {
                if (intval($WPBE->settings->get_site_editor_visibility()[$t->name]) === 0) {
                    continue;
                }
            }
    ...
    <?php foreach ($filter_keys as $field_key => $field) : ?>
        ...
        <div class="col-lg-4">
            <div class='filter-unit-wrap <?php echo $field['css_classes'] ?> <?php if (!$field['direct']): ?>wpbe-direct-field<?php endif; ?>'>
                ...
                <div class="col-lg-11">
                    <?php
                    $opt = array(-1 => sprintf(esc_html('Set: %s', 'bulk-editor'), $field['title']));
