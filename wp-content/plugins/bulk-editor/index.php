<?php
/*
  Plugin Name: Posts Bulk Editor Professional
  Plugin URI: https://wp.bulk-editor.com/
  Description: WPBE - WordPress Posts Bulk Editor Professional. Tools for managing and bulk edit posts, pages and all custom types data in the reliable and flexible way! Be professionals with managing data of your site!
  ...
  Version: 2.0.1.1
  Requires PHP: 5.4
  Tags: bulk,bulk edit,bulk posts,bulk delete
...
include WPBE_PATH . 'classes/rate_alert.php';
...
final class WPBE {
    ...
    private $ext = array('filters', 'bulk', 'export', 'meta', 'history', 'calculator', 'info', 'fprofiles');
...
//***
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'wpbe') {
            ...
            add_action('admin_notices', function() {
                ...
                if (!get_user_meta($user_id, 'wpbe_notice_dismissed')) {
                    echo '<div class="notice notice-warning"><p>' . sprintf(esc_html('If you not familiar with the plugin, firstly %s please', 'bulk-editor'), WPBE_HELPER::draw_link(array(
                                'title' => esc_html('visit this page', 'bulk-editor'),
                                'href' => 'https://wp.bulk-editor.com/document/posts-editor/',
                                'target' => '_blank'
                    ))) . '</p><a href="admin.php?page=wpbe&wpbe-notice-dismissed=1" class="notice-dismiss"></a></div>';
                }
            });
            add_action('admin_init', function() {
                $user_id = get_current_user_id();
                if (isset($_GET['wpbe-notice-dismissed'])) {
        ...
        add_action('admin_bar_menu', function($wp_admin_bar) {
            ...
            if (isset($opt['options']['show_admin_bar_menu_btn']) AND intval($opt['options']['show_admin_bar_menu_btn']) === 1) {
                $args = array(
                    'id' => 'wpbe-btn',
                    'title' => esc_html('WPBE Bulk Editor', 'bulk-editor'),
...
//***
        ...
        $rate_alert= new WPBE_RATE_ALERT(false);
        $rate_alert->init();   
    }
    ...
    public function plugin_action_links($links) {
        ...
        return array_merge(array(
            '<a href="' . admin_url('admin.php?page=wpbe') . '">' . esc_html('Posts Editor', 'bulk-editor') . '</a>',
            '<a target="_blank" href="' . esc_url('https://wp.bulk-editor.com/') . '"><span class="icon-book"></span>&nbsp;' . esc_html('Documentation', 'bulk-editor') . '</a>'
                ), $links);
    }
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'wpbe') {
            ...
            <script>
                var lang = {
                    ...
                    enter_duplicate_count: "<?php printf(esc_html('Enter how many time duplicate selected [%s](s)!', 'bulk-editor'), $this->settings->current_post_type) ?>",
                    enter_new_count: "<?php printf(esc_html('Enter how many new [%s](s) to create!', 'bulk-editor'), $this->settings->current_post_type) ?>",
                    ...
                    action_state_1: "<?php printf(esc_html('all the [%s] on the site', 'bulk-editor'), $this->settings->current_post_type) ?>",
                    action_state_2: "<?php printf(esc_html('the filtered [%s]. To remove filtering press reset button on the tools panel below', 'bulk-editor'), $this->settings->current_post_type) ?>",
                    action_state_31: "<?php printf(esc_html('the selected [%s](s)', 'bulk-editor'), $this->settings->current_post_type) ?>",
                    action_state_32: "<?php printf(esc_html('You can reset selection of the [%s](s) by its reset button on the panel of the editor OR uncheck them manually!', 'bulk-editor'), $this->settings->current_post_type) ?>",
...
//service
    private function __pack_row($p) {
        ...
        $row[] = WPBE_HELPER::draw_link(array(
                    ...
                    'title_attr' => esc_html('View the post on the site front', 'bulk-editor')
                )) . '&nbsp;' . WPBE_HELPER::draw_link(array(
                    'title' => '&#xea25;',
                    'href' => get_admin_url() . 'post.php?post=' . $p['ID'] . '&action=edit',
                    'target' => '_blank',
                    'class' => 'button button-primary button-small',
                    'title_attr' => esc_html('Editing of the post on its page', 'bulk-editor')
        ));
...
//***
            ...
            echo $this->posts->update_page_field($post_id, $field_key, $value);
        }
...
//ajax
    public function wpbe_title_autocomplete() {
        $results = array();
        $results[] = array(
            "name" => esc_html("Posts not found!", 'bulk-editor'),
            "id" => 0,
            "type" => "",
            "link" => "#",
    ...
    public function wpbe_create_new_post() {
        ...
        while ($to_create) {
            wp_insert_post(array(
                ...
                'post_title' => sprintf(esc_html('New %s', 'bulk-editor'), $this->settings->current_post_type),
                'post_status' => 'draft'
            ));
            ...
            foreach ($_REQUEST['posts_ids'] as $post_id) {
                ...
                if (isset($post) && $post != null) {
                    ...
                    $args = array(
                        ...
                        'post_status' => 'publish',
                        'post_title' => $post->post_title . ' (' . esc_html('copy') . ')',
...
//***
        ...
        switch ($this->settings->active_fields[$field_key]['edit_view']) {
            ...
            case 'popupeditor':
                ...
                break;
            /* let it be here, frozen
              case 'gallery_popup_editor':
              $res = WPBE_HELPER::draw_gallery_popup_editor_btn($field_key, $post_id);
              break;
             */
...
//textinput
        ...
        if ($field_key === 'ID') {
            ...
            $title = esc_html('see the post on the site', 'bulk-editor');
