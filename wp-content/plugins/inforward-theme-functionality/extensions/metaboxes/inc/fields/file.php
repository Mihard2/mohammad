...
class MAD_File_Field extends MAD_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		self::localize_script( 'mad-file', 'madFile', array(
			'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'inforward' ),
			'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'inforward' ),
		) );
	}
	...
	public static function wp_ajax_delete_file() {
		...
		wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'inforward' ) );
	}
	...
	public static function html( $meta, $field ) {
		$i18n_title = apply_filters( 'mad_file_upload_string', _x( 'Upload Files', 'file upload', 'inforward' ), $field );
		$i18n_more  = apply_filters( 'mad_file_add_string', _x( '+ Add new file', 'file upload', 'inforward' ), $field );
	...
	protected static function get_uploaded_files( $files, $field ) {
		...
		$classes = 'mad-images mad-uploaded';
		if ( count( $files ) <= 0 ) {
			$classes .= ' hidden';
		}
	...
	protected static function file_html( $file ) {
		$i18n_delete = apply_filters( 'mad_file_delete_string', _x( 'Delete', 'file upload', 'inforward' ) );
		$i18n_edit   = apply_filters( 'mad_file_edit_string', _x( 'Edit', 'file upload', 'inforward' ) );
