 ...
 */
class MAD_Conditional {
	public function __construct() {
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue' ) );
		add_action( 'mad_before', array( $this, 'insert_meta_box_conditions' ) );
		add_filter( 'mad_wrapper_html', array( $this, 'insert_field_conditions' ), 10, 3 );
	}
	...
	public function enqueue( $hook )
	{
		...
		list( , $url ) = MAD_Loader::get_path( dirname(__FILE__) . '/includes/metaboxes' );
		// backward compatibility
		$url = $url . 'js/';

		wp_register_script( 'conditional-logic', plugins_url('/js/conditional-logic.js', dirname(__FILE__) ), array(), '1.4', true );
	...
	public function insert_meta_box_conditions( $obj )
	{
		...
		echo '<div style="display: none; visibility: hidden" class="mad-conditions" data-conditions="'. esc_attr( json_encode( $conditions ) ) . '"></div>';
	}
	...
	public function insert_field_conditions( $begin, $field, $meta )
	{
		...
		$begin .= '<div style="display: none; visibility: hidden" class="mad-conditions" data-conditions="'. esc_attr( json_encode( $conditions ) ) .'"></div>';
	...
	{
		$outside_conditions = apply_filters( 'mad_outside_conditions', array() );
