;(function($)
{
	...
	function isLogicCorrect(logics, $field)
	{
		...
		$.each( logics.when, function( index, logic ) 
		{
			...
			var isDependentFieldVisible = dependentField.closest('.mad-field').attr('data-visible');
	...
	function getFieldScope($field, eventSource)
	{
		...
		var $scope = '', 
			$wrapper = $(eventSource).closest('.mad-clone');
	...
	function runConditionalLogic(conditions)
	{
		...
		$('.mad-conditions').each(function()
		{
			...
			var $selector 		= field.parent().hasClass('mad-metabox-field') ? field.parent() : field.closest('.postbox');
	...
	function applyVisible($element)
	{
		...
		if ($element.closest('.mad-field').length)
			$element = $element.closest('.mad-field');
