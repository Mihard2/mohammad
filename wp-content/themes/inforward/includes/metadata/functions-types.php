...
if (!function_exists('inforward_save_taxdata')) {

	function inforward_save_taxdata( $term_id, $ts_id, $taxonomy, $meta_boxes ) {
...
if (!function_exists('inforward_show_tax_add_meta_html')) {

	// Show Taxonomy Add meta
	function inforward_show_tax_add_meta_html($meta_box) {
		...
		<div class="inforward-form-field">
			...
			if ($type == "select") :  ?>
				...
				<select name="<?php echo esc_attr($name) ?>" id="<?php echo esc_attr($name) ?>">
					...
					<?php if ($options == 'page'): ?>
						...
						<option value=""><?php esc_html_e('None', 'inforward'); ?></option>
						...
						if (is_array($options)) :
							foreach ($options as $key => $entry) :
								...
								?>
								<option value="<?php echo esc_attr($std) ?>"><?php echo esc_attr($title) ?></option>
							<?php endforeach;
						endif; ?>
			...
			if ($type == "upload") :  ?>
				...
				<label for='upload_image'>
					...
					<button class="button_upload_image button" id="<?php echo esc_attr($name) ?>"><?php esc_html_e('Upload Image', 'inforward') ?></button>
					<button class="button_remove_image button" id="<?php echo esc_attr($name) ?>"><?php esc_html_e('Remove Image', 'inforward') ?></button>
				</label>
				<?php if ($desc) : ?><p><?php echo esc_html($desc) ?></p><?php endif; ?>
			<?php endif;
			...
			if (($type == 'radio') && (!empty($options))) :  ?>
				...
				<?php foreach ($options as $key => $value) : ?>
					...
					<label style="display:inline-block" for="<?php echo esc_attr($name) ?>_<?php echo esc_attr($key) ?>"><?php echo esc_attr($value) ?></label>
				<?php endforeach; ?>
				<?php if ($desc) : ?><p><?php echo esc_html($desc) ?></p><?php endif; ?>
			<?php endif; ?>
...
if (!function_exists('inforward_show_tax_edit_meta_html')) {

	// Show Taxonomy Edit meta
	function inforward_show_tax_edit_meta_html($term, $taxonomy, $meta_box) {
		...
		<tr class="inforward-form-field">
				...
				<td>
					<select name="<?php echo esc_attr($name) ?>" id="<?php echo esc_attr($name) ?>">
						...
						<?php if ($options == 'page'): ?>
							...
							if (is_array($options)) :
								foreach ($options as $key => $entry) :
									...
									$title = $entry->post_title;
									?>
									<option value="<?php echo esc_attr($std) ?>"<?php echo esc_attr($meta_box_value) == $std ? ' selected="selected"' : '' ?>><?php echo esc_attr($title) ?></option>
								<?php endforeach;
							endif; ?>
						...
						<?php else: ?>
							<?php if (is_array($options)) :
								foreach ($options as $key => $value) : ?>
									<option value="<?php echo esc_attr($key) ?>"<?php echo esc_attr($meta_box_value) == $key ? ' selected="selected"' : '' ?>><?php echo esc_attr($value) ?></option>
								<?php endforeach;
							endif; ?>
						<?php endif; ?>
			...
			if ($type == "editor") : ?>
				<th colspan="2" scope="row" valign="top"><label for="<?php echo esc_attr($name) ?>"><?php echo esc_attr($title) ?></label></th>
				<td colspan="2">
			...
			if ($type == "upload") : ?>
				<th scope="row" valign="top"><label for="<?php echo esc_attr($name) ?>"><?php echo esc_attr($title) ?></label></th>
				<td>
					<label for='upload_image'>
			...
			if ($type == "checkbox") : ?>
				...
				<th scope="row" valign="top"><label for="<?php echo esc_attr($name) ?>"><?php echo esc_attr($title) ?></label></th>
				<td>
					<label><input type="checkbox" name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($name) ?>" <?php echo wp_kses_post($checked) ?> /> <?php echo esc_html($desc) ?></label>
				</td>
			<?php endif;
			...
			if (($type == 'radio') && (!empty($options))) : ?>
				...
				<td>
					<?php foreach ($options as $key => $value) : ?>
						...
						<label for="<?php echo esc_attr($name) ?>_<?php echo esc_attr($key) ?>"><?php echo esc_attr($value) ?></label>
					<?php endforeach; ?>
					<?php if ($desc) : ?><p class="description"><?php echo esc_html($desc) ?></p><?php endif; ?>
				</td>
			<?php endif; ?>
