<?php
if (!class_exists('INFORWARD_METADATA_PRODUCT')) {

	class INFORWARD_METADATA_PRODUCT {
		...
		public function __construct() {
			...
			$this->paths = array(
				...
				'BASE_URI' => get_template_directory() . '/metadata'
			);
		...
		function add_enqueue_styles() {
			...
			wp_enqueue_style( 'inforward-metadata-mod', $css_file );
		}
		...
		function show_tax_add_meta_boxes($meta_boxes) {
			...
			foreach ($meta_boxes as $meta_box) {
				inforward_show_tax_add_meta_html($meta_box);
			}
		}
		...
		function show_tax_edit_meta_boxes($term, $taxonomy, $meta_boxes) {
			...
			foreach ($meta_boxes as $meta_box) {
				inforward_show_tax_edit_meta_html($term, $taxonomy, $meta_box);
			}
		}
		...
		function product_get_postdata() {
			global $product_cat_meta_array;
			$product_cat_meta_array = inforward_cat_meta_view();
		}
		...
		function save_product_cat($term_id, $tt_id, $taxonomy) {
			...
			return inforward_save_taxdata( $term_id, $tt_id, $taxonomy, $product_cat_meta_array );
		}

	}

	new INFORWARD_METADATA_PRODUCT();
}