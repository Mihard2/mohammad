...
if ( ! class_exists('Inforward_Envato_API') ) :
	...
	class Inforward_Envato_API {
		...
		/**
		 * Main Inforward_Envato_API Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see Inforward_Envato_API()
		 * @uses Inforward_Envato_API::init_globals() Setup class globals.
		 * @uses Inforward_Envato_API::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true Inforward_Envato_API.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see Inforward_Envato_API::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'inforward' ), '1.0.0' );
		}
		...
		private function init_globals() {
			// Envato API token.
			self::$token = Inforward()->registration->get_token();
		}
		...
		public function request( $url, $args = array() ) {
			$defaults = array(
				'headers' => array(
					...
					'User-Agent' => 'WordPress - Inforward ' . Inforward()->get_theme_version(),
				),
				'timeout' => 20,
			);
			...
			if ( empty( self::$token ) ) {
				return new WP_Error( 'api_token_error', esc_html__( 'An API token is required.', 'inforward' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, esc_html__( 'An unknown API error occurred.', 'inforward' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', esc_html__( 'An unknown API error occurred.', 'inforward' ) );
				}
				return $return;
			}
		}
		...
		public function deferred_download( $id ) {
			...
			return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=inforward' ) ) );
		}
