...
window.InforwardTabView = vc.shortcode_view.extend({
	...
	initialize:function (params) {
            window.InforwardTabView.__super__.initialize.call(this, params);
            _.bindAll(this, 'stopSorting');
           //console && console.log('InforwardTabView: initialize method called.');
        },
	// Render method called after element is added( cloned ), and on first initialisation
	render: function () {
        //console && console.log('InforwardTabView: render method called.');
		window.InforwardTabView.__super__.render.call(this); //make sure to call __super__. To execute logic fron inherited view. That way you can extend original logic. Otherwise, you will fully rewrite what VC will do at this event
		this.$tabs = this.$el.find('.wpb_tabs_holder');
        this.createAddTabButton();
        //console &&  console.log('InforwardTabView '+this.$tabs);
		return this;
	},
	ready: function (e) {
        //console && console.log('InforwardTabView: ready method called.');
		window.InforwardTabView.__super__.ready.call(this, e);
    ...
    createAddTabButton:function(){
    	var new_tab_button_id = (+new Date() + '-' + Math.floor(Math.random() * 11));
         ...
         //console &&  console.log(' InforwardTabView : createAddTabButton');
        },
    addTab:function (e) {
    	//console &&  console.log('addTab');
            ...
            var tab_title = window.i18nLocale.tab,
                tabs_count = this.$tabs.find('[data-element_type=vc_mad_single_tab]').length,
                tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
            vc.shortcodes.create({shortcode:'vc_mad_single_tab', params:{title:tab_title, tab_id:tab_id}, parent_id:this.model.id});
            //console &&  console.log(' InforwardTabView :  addTab');
            return false;
        },
        stopSorting:function (event, ui) {
            var shortcode;
             //console &&  console.log(' InforwardTabView :  stopSorting loading');
            this.$tabs.find('ul.tabs_controls li:not(.add_tab_block)').each(function (index) {
              ...
              //console &&  console.log(' InforwardTabView :  shortcode save');
            shortcode.save();
        },
       changedContent:function (view) {
            ...
            //console &&  console.log(' InforwardTabView :  changedContent save');
        },
   cloneModel:function (model, parent_id, save_order) {
            ...
            if (model.get('shortcode') === 'vc_mad_single_tab') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element-type=vc_mad_single_tab]').length + '-' + Math.floor(Math.random() * 11)});
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'vc_mad_single_tab' ? false : true), cloned_from:model.toJSON(), params:new_params});
...
// Come from vc_map -> 'js_view' => 'InforwardTabView'
})(window.jQuery);
