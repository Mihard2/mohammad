...
window.InforwardSubTabView = window.VcColumnView.extend({
	events:{
	...
	render:function () {
		...
		window.InforwardSubTabView.__super__.render.call(this);
		if(!params.tab_id) {
	...
	ready:function (e) {
		...
		window.InforwardSubTabView.__super__.ready.call(this, e);
		this.$tabs = this.$el.closest('.wpb_tabs_holder');
		var params = this.model.get('params');
		//console && console.log('InforwardSubTabView.ready() loading');
		return this;
	},
	changeShortcodeParams:function (model) {
		var params = model.get('params');
		window.InforwardSubTabView.__super__.changeShortcodeParams.call(this, model);
		//var params = model.get('params');
		...
		//console && console.log('InforwardSubTabView changeShortcodeParams loading');
	},
	deleteShortcode:function (e) {
		...
		// console && console.log('InforwardSubTabView.deleteShortcode loading');
	},
	cloneModel:function (model, parent_id, save_order) {
		...
		if (model.get('shortcode') === 'vc_mad_single_tab') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element_type=vc_mad_single_tab]').length + '-' + Math.floor(Math.random() * 11)});
		...
		//console && console.log('InforwardSubTabView.cloneModel loading');
		return model_clone;
	}
