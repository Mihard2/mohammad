...
$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, ' cp-table-holder wpb_mad_table wpb_content_element'.$el_class, $this->settings['base']);

echo '<div class="'.esc_attr($css_class).' cp-table-'.esc_attr($vc_table_theme).'">';
echo '<table'.$custom_css.'>';
foreach($table_data as $index => $row) {
    echo '<tr'.($index===0 ? ' class="vc-th"' : '').'>';
    foreach($row as $cell) {
	    ...
	    echo '<td' . $style . $class . '><span class="vc_table_content">' . $cell_content . '</span></td>';
    }
    echo '</tr>';
}
echo '</table></div>';
