(function ($, window) {
	...
	function inforward_sidebar_generator () {
		var base = this;
			base.widgetArea = $('#widgets-right');
			base.widgetTemplate  = $('#inforward-tmpl-add-widget');
	...
	inforward_sidebar_generator.prototype = {
		createForm: function ()  {
            this.widgetLiquid.append(this.widgetTemplate.html());
            this.nonce = this.widgetLiquid.find('input[name="inforward-custom-sidebar-nonce"]').val();
        },
		createRemoveButton: function () {
            this.widgetArea.find('.sidebar-inforward-widget-custom').append('<span class="custom-sidebar-delete dashicons-trash"></span>');
        },
