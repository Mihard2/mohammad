...
"use strict"; 
	...
	$.inforward_widget_metaBox = function () {
		return ({
			init: function () {
				var base = this;
					base.widgetTemplate = $('#inforward-tmpl-options-hidden');
					base.listeners();
			},
			listeners: function () {
				var base = this,
				...
				$(".meta-list-set ul.options-columns").on('click', 'li', function () {
					...
					for (var i = 0; i < obj[val][0].length; i++) {
				...
				$(".meta-list-set ul.options-type").on('click', 'li', function () {
					...
					for (var i = 0; i < obj[all_val][(val - 1)].length; i++) {
				...
				$('[name="footer_row_top_show"], [name="footer_row_middle_show"]').on('click', function () {
					var $this = $(this),
						value = $this.val(),
